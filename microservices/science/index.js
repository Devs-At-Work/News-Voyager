...
app.use('/science', (req, res, next) => {
  // Remove /science from the path before processing
  req.url = req.url.replace(/^\/science/, '');
  next();
});
...
app.get('/science/health', (req, res) => {
...
app.get('/science/news/:category', async (req, res) => {
  try {
